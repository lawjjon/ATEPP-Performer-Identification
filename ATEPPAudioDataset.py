...
class ATEPPAudioDataset(data.Dataset):
    def __init__(self, metadata_dir, target_sample_rate, num_samples, split, num_chunks, is_augmentation):
        self.metadata_dir = metadata_dir + '/' if metadata_dir else ''
        self.split = split
        self.annotations_file = self.metadata_dir + '%s_discog_filtered_over_115_balanced.csv' % split
    ...
    def __getitem__(self, index):
        label = self.annotations.iloc[index, 8]
    ...
    def _get_audio_sample_path(self, index):
      piece_path = self.annotations.iloc[index, 4]
      return piece_path
